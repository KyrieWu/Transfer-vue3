...
export function useCheckedData() {
  ...
  function removeCheakedData(leftOrRight, id) {
    switch (leftOrRight) {
      case "left":
  ...
  return [checkedData, addCheckedData, removeCheakedData];
}

export function useDragedItem() {
  const dragedItem = ref(null);

  function setDregedItem(item) {
    dragedItem.value = item;
  }

  return [dragedItem, setDregedItem];
}
